    ...
    steps:
      ...
      - name: Upload to VOE.sx (1080p only, direct file upload)
        ...
        run: |
          ...
          UPLOAD_SERVER=$(echo "$SERVER_RESPONSE" | python3 -c "
          import sys, json
          try:
              ...
              raise SystemExit
          r = d.get('result', '')
          # Confirmed real shape: result IS the upload URL (a string).
          # Kept the dict fallback in case VOE ever nests it instead.
          if isinstance(r, dict):
              r = r.get('upload_server', '')
